...
export type OrbAgentState = AgentState;
    ...
    useFrame((_, delta: number) => {
        ...
        if (modeRef.current === 'manual') {
            ...
            if (agentRef.current === null) {
                ...
                const wander =
                    0.05 * Math.sin(t * 2.1) * Math.sin(t * 0.37 + 1.2);
        ...
        animSpeedRef.current +=
            (targetSpeed - animSpeedRef.current) * 0.12;
    ...
    const uniforms = useMemo(() => {
        ...
        return {
            uColor1: new THREE.Uniform(
                new THREE.Color(initialColorsRef.current[0])
            ),
            uColor2: new THREE.Uniform(
                new THREE.Color(initialColorsRef.current[1])
            ),
